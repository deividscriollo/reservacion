<?php 
	...
	if (isset($_POST['llenar_clientes'])) {
		...
		$resultado = $class->consulta("SELECT CEDULA,NOMBRE,FONO,DIRECCION,U.ID FROM SEG.USUARIO U,SEG.NIVEL N WHERE N.ID_USUARIO=U.ID  AND U.STADO='1'");		
		$acu;
		while ($row=$class->fetch_array($resultado)) {					
			$acu[]=$row[0];
			$acu[]=$row[1];
	...
	if (isset($_POST['llenar_reservacion_datos'])) {
		$resultado = $class->consulta("SELECT R.ID,CEDULA,S.NOM,HINICIo,HFIN,FE FROM SERVICIOS S, RESERVACION R, SEG.USUARIO U, RESERVACION_HORARIOS RH WHERE R.ID=RH.ID_RESERVACION AND S.ID=R.ID_SERVICIO AND U.ID=R.ID_USUARIO AND R.STADO='0' and U.ID='".$_POST['id']."'");				
		while ($row=$class->fetch_array($resultado)) {
			print'<tr>';
			print'<td>'.$row[1].'</td>';
			print'<td>'.$row[2].'</td>';
			...
			print'<td>'.$row[5].'</td>';
			print'<td>'.'<button class="btn btn-mini btn-danger" onclick="seleccion_reservacion('."'".$row[0]."'".')">
						<i class="icon-arrow-right  icon-on-right"></i>
					</button>'.'</td>';
			print'</tr>';
	 	}
	...
	if (isset($_POST['datos_factura_tarifa'])) {
		$resultado = $class->consulta("SELECT T.SERVICIOS,T.PRECIO,T.CANTIDAD,T.TOTAL,SUBTOTAL
										FROM RESERVACION R, SERVICIOS S, RESERVACION_TARIFA T
										WHERE R.ID_SERVICIO=S.ID AND T.ID_RESERVACION=R.ID AND T.CANTIDAD!=0 AND R.ID='".$_POST['id']."'");				
		while ($row=$class->fetch_array($resultado)) {	
			$acu[]=$row[0];
			$acu[]=$row[1];
			$acu[]=$row[2];			
